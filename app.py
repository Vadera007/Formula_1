...
.main-content {
    padding: 0 5vw 80px 5vw; /* Added more bottom padding for footer */
    max-width: 1200px;
    margin: auto;
}
...
DB_FILE = os.path.join(BASE_DIR, "..", "data", "f1_predictor.db")
XGB_MODEL_FILE = os.path.join(BASE_DIR, "..", "Trinity", "xgb_model.joblib")
LGBM_MODEL_FILE = os.path.join(BASE_DIR, "..", "Trinity", "lgbm_model.joblib")
CATBOOST_MODEL_FILE = os.path.join(BASE_DIR, "..", "Trinity", "catboost_model.joblib")
...
if df is None:
    st.warning("Model or data files not found. Please run `run_pipeline.py` first.")
    st.stop()
...
if next_race is not None:
    ...
    with st.spinner("Firing up the ensemble engine..."):
        ...
        # Use the last race in the dataset as a clean template
...
if os.path.exists(DB_FILE):
    last_updated_time = datetime.fromtimestamp(os.path.getmtime(DB_FILE)).strftime('%B %d, %Y at %I:%M %p %Z')
    footer_text = f"""
    <div class="footer">
        <p>© 2025 F1 Race Predictor | A Project by Akshat Vadera</p>
        <p style="font-size: 12px; color: #888;">Last Data Update: {last_updated_time}</p>
    </div>
    """
else:
    footer_text = """
